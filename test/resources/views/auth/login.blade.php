...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0f172a] to-[#1e293b] text-white font-sans">
    <div class="w-full max-w-md bg-[#1e293b] p-8 rounded-xl shadow-xl border border-white/10">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            @csrf
            <div>
                <label for="username" class="block font-bold text-cyan-400 mb-1">username</label>
                <input type="text" name="username" id="username" placeholder="exemple@gmail.com" class="w-full px-4 py-2 rounded bg-[#334155] font-bold text-gray-200 placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-cyan-400" required/>
                @error('username')
